import 'package:dartz/dartz.dart';
import 'package:flutter_advanced_course/data/network/failure.dart';
import 'package:flutter_advanced_course/domain/repository/repository.dart';
import 'package:flutter_advanced_course/domain/usecase/base_use_case.dart';

class PutCancelBookingsUseCase implements BaseUseCase<int, String> {
  final Repository _repository;
  PutCancelBookingsUseCase(this._repository);

  @override
  Future<Either<Failure, String>> execute(int input) async {
    return await _repository.putCancelBookings(input);
  }
}
